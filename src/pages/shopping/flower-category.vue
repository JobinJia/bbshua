  ...
  export default {
        ...
        tableTitle: [
          {
            ...
            render: (h, params) => {
              return <div style="display:flex;flexDirection:row;justify-content:space-around;align-items:center;">
                ...
                <i-button type="primary" size="small" nativeOnClick={this.viewItem.bind(this, params.row)}>查看</i-button>
    ...
    methods: {
      ...
      async getList () {
        ...
        /* eslint-disable */
        let {data} = await this.$http.getCategory(query)
        this.tableData = data
      },
      editorHandle (row) {
        this.item = row
      ...
      async successHandler () {
        ...
        let {code} = await this.$http.updCategory(query)
        if (code === 0) {
      ...
      async deleteItem (row) {
        this.$Modal.confirm({
          ...
          onOk: async () => {
            ...
            let {code} = await this.$http.delCategory(query)
            if (code === 0) {
              this.$Message.success({
                content: '删除成功！'
              })
      ...
      async addHandler () {
        ...
        let {code} = await this.$http.addCategory(query)
        if (code === 0) {
          this.$Message.success({
            content: '新增成功！'
          })
