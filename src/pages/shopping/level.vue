  ...
  export default {
    ...
    methods: {
      ...
      async getList () {
        ...
        /* eslint-disable */
        let {data} = await this.$http.getLevelList(query)
        this.tableData = data
      },
      editorHandle (row) {
        this.item = row
      ...
      async successHandler () {
        ...
        let {code} = await this.$http.commitLevel(query)
        if (code === 0) {
      ...
      async deleteItem (row) {
        this.$Modal.confirm({
          ...
          onOk: async () => {
            ...
            let {code} = await this.$http.deleteLevel(query)
            if (code === 0) {
              this.$Message.success({
                content: '删除成功！'
              })
      ...
      async addHandler () {
        ...
        let {code} = await this.$http.addLevel(query)
        if (code === 0) {
          this.$Message.success({
            content: '新增成功！'
          })
